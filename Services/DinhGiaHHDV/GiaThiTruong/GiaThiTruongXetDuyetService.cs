...
{
    public class GiaThiTruongXetDuyetService(ApplicationDbContext dbContext) : IGiaThiTruongXetDuyetService
    {
        private readonly ApplicationDbContext _dbContext = dbContext;

        public async Task<CommonResponse> GetListXetDuyetByFilterAsync(int year, string thang, Guid donViId, bool isSSA, string search, int pageSize, int pageCurrent)
        {
            try
            {
        ...
        public async Task<CommonResponse> DuyetAsync(Guid id)
        {
            try
            {
                var model = await _dbContext.GiaThiTruongs.FirstOrDefaultAsync(t => t.Id == id);
        ...
        public async Task<CommonResponse> HuyCongBoAsync(Guid id)
        {
            try
            {
                ...
                model.TrangThai = "HCB"; // Hủy công bố
                model.UpdatedDate = DateTime.Now;
                _dbContext.GiaThiTruongs.Update(model);
                await _dbContext.SaveChangesAsync();
